...
def load_ema_model(model, resume_model):
    if resume_model is not None:
        logger.info(f'Load ema model from {resume_model}')
        if not os.path.exists(resume_model):
            raise ValueError(
                f'Directory of the model needed to resume is not Found: {resume_model}'
            )
...
def save_edge(edges_src, name):
    ...
    tmp[tmp == 1] == 255
    imageio.imwrite(f'edge_pics/edge_{name}.png', tmp)
